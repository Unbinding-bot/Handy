...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.inactive || state == AppLifecycleState.paused) {
      ...
      appController.toggleCursorVisibility(false); // Hide native cursor on pause
    } else if (state == AppLifecycleState.resumed) {
      _initializeCamera(appController.selectedCameraIndex);
      if (appController.isCursorVisible) {
          appController.toggleCursorVisibility(true); // Re-show native cursor
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      // Keep the main layout as a Stack to handle the floating cursor
      body: Stack(
        children: [
          // 1. Main UI Structure (Uses a Column to enforce sequential layout)
          Container(
             // Set the background color to be consistent if camera is off
            ...
            child: SafeArea(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    // 1.1. TOP BAR (Settings and Status) - Static
                    Row(
                    ...
                    const SizedBox(height: 20), // Spacer after top bar

                    // 1.2. CONTROL TOGGLE BUTTON - Static
                    Center(
                    ...
                    const SizedBox(height: 20), // Spacer before optional camera feed

                    // 1.3. CAMERA FEED DISPLAY - Dynamic (Below Power Button)
                    if (controller.isCameraPreviewVisible && _cameraController != null && _cameraController!.value.isInitialized)
                      Expanded( // Allow camera display to take available vertical space
                        child: ClipRRect(
                          borderRadius: BorderRadius.circular(20), // Rounded corners for the container
                          child: Container(
                            color: Colors.black, // Background when aspect ratio is different
                            child: FittedBox(
                              fit: BoxFit.contain,
                              child: SizedBox(
                                // Use previewSize and swap dimensions for correct aspect ratio.
                      ...
                      // Use a Spacer if camera is off, pushing the control box to the bottom
                      const Spacer(), 


                    // 1.4. SHOW CURSOR/CAMERA FEED CONTROLS - Dynamic (Always at the bottom)
                    Container(
                      // Only add top margin if the camera feed is NOT visible, 
                      // otherwise the Expanded widget above takes up the space.
                      margin: controller.isCameraPreviewVisible ? EdgeInsets.zero : const EdgeInsets.only(top: 20),
          ...
          // 2. Flutter Cursor Overlay (Floating on top of everything)
          if (controller.isControlActive && controller.isCursorVisible)
            AnimatedPositioned(
              duration: const Duration(milliseconds: 100),
              // Use dx/dy directly as they represent logical Flutter pixels
